...
export default function IntegersGame({ onPhaseChange, onExit, setupHeaderExtra }) {
  ...
  function handleMistakesOnly() {
    ...
    setSettings({ law: 'weak', weakQuestions: mistakes.map((a) => a.question) })
    setRoundKey((k) => k + 1)
    setStage('playing')
  }
  ...
  return (
    <div className="game-shell">
      {stage === 'setup' && (
        <SetupWizard initialSettings={settings} onComplete={handleSetupComplete} headerExtra={setupHeaderExtra} />
      )}
      {stage === 'playing' && (
